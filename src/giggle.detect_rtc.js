...
//Hardware detection for IE and Safari

function detectHardwareForIEAndSafari() {
  var has_cam = false;
  var has_mic = false;

  //Checking if 'MediaStreamTrack' is defined and also if it has 'getSources' function
  if( GIGGLE_IS_PluginRTC && typeof(MediaStreamTrack) == "object" && typeof(MediaStreamTrack.getSources) == "function" ) {
    var sources = MediaStreamTrack.getSources(
      function(source) {
        return source;
      }
    );
    ...
    jQuery.each(sources, function(i, source) {
      // For some browsers like IE Edge, source is always present with "id = default",
      // even when no hardware is connected. We need to exclude them.

      if(source.kind === "audio" && source.id.indexOf("default") == -1)
        has_mic = true;

      if(source.kind === "video" && source.id.indexOf("default") == -1)
        has_cam = true;
    });
  }

  return {has_mic: has_mic, has_cam: has_cam};
}
